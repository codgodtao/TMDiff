...
class GeneralDiffusion(nn.Module):
    ...
    def classifier_free_guidance_sample(self, w, x, noise_level, x_in):
        ...
        PAN = x_in['PAN']
        n_spectral = MS.shape[1]
        pan_concat = PAN.repeat(1, n_spectral, 1, 1)
    ...
    @torch.no_grad()
    def p_mean_variance(self, x, t, clip_denoised=True, x_in=None, prompt="QB"):
        ...
        if x_in is not None:
            x_recon = self.predict_start_from_noise(  # 根据噪声得到x0
                x, t=t, noise=self.denoise_fn(x_t=x, t_input=time_in, MS=x_in['MS'], PAN=x_in['PAN'], prompt=prompt))
    ...
    @torch.no_grad()
    def p_sample(self, x, t, clip_denoised=True, condition_x=None, prompt="QB"):
        # 在给定x,t和yt的条件下，获得yt-1的和均值和方差，值得注意的是估计的X0已经被归一化，通过公式计算yt-1
        model_mean, model_log_variance = self.p_mean_variance(
            x=x, t=t, clip_denoised=clip_denoised, x_in=condition_x, prompt=prompt)
        noise = torch.randn_like(x) if t > 0 else torch.zeros_like(x)
        return model_mean + noise * (0.5 * model_log_variance).exp()

    @torch.no_grad()
    def p_sample_loop(self, x_in, continous=False, prompt="QB"):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step',
            ...
            img = self.p_sample(img, i, condition_x=x_in, prompt=prompt)  # 在给定xt,t,condition，采样得到xt-1
    ...
    @torch.no_grad()
    def super_resolution(self, x_in, continous=False, prompt="QB"):  # inference接口，会获取多个（8个）逐渐去噪的结果，并可以绘制一个过程图像
        return self.p_sample_loop(x_in, continous, prompt)
    ...
    def p_losses_dynamic(self, x_in, prompt="QB"):
        # 训练过程使用的L1损失函数
        x_start = x_in['HR']
        ...
        # print(time_in.shape, x_start.shape, continuous_sqrt_alpha_cumprod.shape, noise.shape)
        x_noisy = self.q_sample(  # 对X0进行加噪得到xt，拓展[b,1]维度为[b,1,1,1],才可以和[b,c,h,w]的x_start,noise进行运算
            x_start=x_start, continuous_sqrt_alpha_cumprod=continuous_sqrt_alpha_cumprod.view(-1, 1, 1, 1), noise=noise)

        x_recon = self.denoise_fn(x_noisy, time_in, x_in['MS'], x_in['PAN'], prompt)
