...
class GeneralDiffusion(nn.Module):
    ...
    def super_resolution(self, x_in, continous, prompt, guidance):
        return self.sample_by_dpmsolver(x_in, prompt)
    ...
    def p_losses_dynamic(self, x_in, prompt=None):
        ...
        # 预测XO, 即[-1,1]分布的Res 或者预测噪声，然后重建X0
...
class GeneralDiffusion2(nn.Module):
    def __init__(
            self,
            denoise_fn,
            loss_type='l1'
    ):
        super(GeneralDiffusion2, self).__init__()
        self.denoise_fn = denoise_fn
        self.loss_type = loss_type
    ...
    def p_losses_dynamic(self, x_in, prompt=None):
        ...
        x_recon = self.denoise_fn(x_noisy, time_in, x_in['PAN'], x_in['MS'], x_in['wav'], prompt)
        # noise predict function
        loss = self.loss_func(x_start, x_recon)
        return loss
