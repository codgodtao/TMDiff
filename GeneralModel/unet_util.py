...
class ChannelAttentionBlock(nn.Module):
    ...
    def forward(self, inputs):
        # b C bands H W
        batch_size, channels, bands, height, width = inputs.size()
        # input channel替换  B H W  bands C
        permute_inputs = inputs.permute(0, 3, 4, 2, 1)
        proj_query = permute_inputs.contiguous().view(batch_size, -1, bands * channels)  # B HW bands*C
        proj_key = proj_query.permute(0, 2, 1)  # B bands*C HW
        energy = torch.bmm(proj_query, proj_key)  # c*bands  c*bands
        attention = F.softmax(energy, dim=-1)  # c*bands  c*bands

        outputs = torch.bmm(attention, proj_query)  # B HW bands*C
        outputs = outputs.view(batch_size, height, width, bands, channels)
        # channel替换回来b C bands H W
...
class ChannelAttention(nn.Module):
    def __init__(self, channel_in, embed_dim):
    ...
    def forward(self, x, embed, context):
        h = x * self.dense1(context).unsqueeze(-1) + self.dense2(context).unsqueeze(-1)
        h = self.attention(self.act(h))
        return h
...
class ResblockDownOneModulateBEST(nn.Module):
    def __init__(self, channel_in, channel_out, embed_dim):
        ...
        self.conv20 = ResBlockModulate4(channel_in, channel_out, embed_dim)
        self.act = Swish()
...
class ResBlockModulateFinalBEST(nn.Module):
    def __init__(self, channel_in, channel_out, embed_dim):
        "(b,32,h,w,c)->(b,32,h,w,c)"
        super(ResBlockModulateFinalBEST, self).__init__()
        self.conv20 = nn.Conv3d(in_channels=channel_in, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
        self.conv21 = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
...
class FinalBlockModulateBEST(nn.Module):
    def __init__(self, channel_out, embed_dim):
        ...
        self.conv20 = ResBlockModulateFinalBEST(channel_out, channel_out, embed_dim)
        self.conv21 = ResBlockModulateFinalBEST(channel_out, channel_out, embed_dim)
        self.conv22 = ResBlockModulateFinalBEST(channel_out, channel_out, embed_dim)
        self.final = nn.Conv3d(in_channels=channel_out,
    ...
    def forward(self, x, embed, context):
        ...
        h = self.conv22(h, embed, context)
        h = self.act(h)
        h = modulated_conv3d(x=h, w=self.final.weight, s=self.dense2(context),
                             stride=self.final.stride, padding=self.final.padding)
        return h


# 模型size的进一步调整，time_steps的微调； 以及基于2T step采样与dpm-solver的区别，solver有无提升空间等
class AdaptionModulate4(nn.Module):
    def __init__(self, channel_in, channel_out, embed_dim):
        super(AdaptionModulate4, self).__init__()
        self.conv20 = nn.Conv3d(in_channels=channel_in, out_channels=channel_out, kernel_size=1,
                                stride=1, padding=0)  # 通道扩张
        self.conv21 = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 0, 0))
...
class ResblockDownOneModulate4(nn.Module):
    def __init__(self, channel_in, channel_out, embed_dim):
        "(b,32,h,w,c)->(b,64,h/2,w/2,c)"
        super(ResblockDownOneModulate4, self).__init__()
        self.down = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                              stride=(1, 2, 2), padding=(1, 0, 0))  # down_sampling
        self.conv20 = ResBlockModulate4(channel_in, channel_out, embed_dim)
        self.act = Swish()
...
class ResblockUpOneModulate4(nn.Module):
    def __init__(self, channel_in, channel_out, embed_dim, kernel_size=(3, 3, 3),
                 stride=(1, 2, 2), out_padding=(0, 1, 1), padding=(1, 0, 0)):
        "(b,32,h,w,c)->(b,64,h/2,w/2,c)"
        super(ResblockUpOneModulate4, self).__init__()
        self.up1 = nn.ConvTranspose3d(in_channels=channel_out, out_channels=channel_out,
                                      kernel_size=kernel_size, stride=stride,
                                      output_padding=out_padding, padding=padding)
        self.act = Swish()
        self.conv20 = ResBlockModulate4(channel_in * 3, channel_out, embed_dim)
...
class ResBlockModulate4(nn.Module):
    def __init__(self, channel_in, channel_out, embed_dim):
        "(b,32,h,w,c)->(b,32,h,w,c)"
        super(ResBlockModulate4, self).__init__()
        self.conv20 = nn.Conv3d(in_channels=channel_in, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
        self.conv21 = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
...
class ResBlockModulateFinal4(nn.Module):
    def __init__(self, channel_in, channel_out, embed_dim):
        "(b,32,h,w,c)->(b,32,h,w,c)"
        super(ResBlockModulateFinal4, self).__init__()
        self.conv20 = nn.Conv3d(in_channels=channel_in, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
        self.conv21 = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
...
class FinalBlockModulate4(nn.Module):
    def __init__(self, channel_out, embed_dim):
        "(b,32,h,w,c)->(b,32,h,w,c)"
        super(FinalBlockModulate4, self).__init__()
        self.conv20 = ResBlockModulateFinal4(channel_out, channel_out, embed_dim)
        self.conv21 = ResBlockModulateFinal4(channel_out, channel_out, embed_dim)
        self.conv22 = ResBlockModulateFinal4(channel_out, channel_out, embed_dim)
        self.final = nn.Conv3d(in_channels=channel_out,
                               out_channels=1, kernel_size=1, stride=1, padding=0)
        self.dense2 = Dense(embed_dim, channel_out)
        self.act = Swish()
...
# no prompt model| no time model
class AdaptionModulate(nn.Module):
    def __init__(self, channel_in, channel_out):
        super(AdaptionModulate, self).__init__()
        self.conv20 = nn.Conv3d(in_channels=channel_in, out_channels=channel_out, kernel_size=1,
                                stride=1, padding=0)  # 通道扩张
        self.conv21 = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 0, 0))
...
class ResblockDownOneModulate(nn.Module):
    def __init__(self, channel_in, channel_out):
        "(b,32,h,w,c)->(b,64,h/2,w/2,c)"
        super(ResblockDownOneModulate, self).__init__()
        self.down = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                              stride=(1, 2, 2), padding=(1, 0, 0))  # down_sampling
        self.conv20 = ResBlockModulate(channel_in, channel_out)
        self.act = Swish()
...
class ResblockUpOneModulate(nn.Module):
    def __init__(self, channel_in, channel_out, kernel_size=(3, 3, 3),
                 stride=(1, 2, 2), out_padding=(0, 1, 1), padding=(1, 0, 0)):
        "(b,32,h,w,c)->(b,64,h/2,w/2,c)"
        super(ResblockUpOneModulate, self).__init__()
        self.up1 = nn.ConvTranspose3d(in_channels=channel_out, out_channels=channel_out,
                                      kernel_size=kernel_size, stride=stride,
                                      output_padding=out_padding, padding=padding)
        self.act = Swish()
        self.conv20 = ResBlockModulate(channel_in * 3, channel_out)
...
class ResBlockModulate(nn.Module):
    def __init__(self, channel_in, channel_out):
        "(b,32,h,w,c)->(b,32,h,w,c)"
        super(ResBlockModulate, self).__init__()
        self.conv20 = nn.Conv3d(in_channels=channel_in, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
        self.conv21 = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
...
class ResBlockModulateFinal(nn.Module):
    def __init__(self, channel_in, channel_out):
        "(b,32,h,w,c)->(b,32,h,w,c)"
        super(ResBlockModulateFinal, self).__init__()
        self.conv20 = nn.Conv3d(in_channels=channel_in, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
        self.conv21 = nn.Conv3d(in_channels=channel_out, out_channels=channel_out, kernel_size=(3, 3, 3),
                                stride=1, padding=(1, 1, 1))
...
class FinalBlockModulate(nn.Module):
    def __init__(self, channel_out):
        "(b,32,h,w,c)->(b,32,h,w,c)"
        super(FinalBlockModulate, self).__init__()
        self.conv20 = ResBlockModulateFinal(channel_out, channel_out)
        self.conv21 = ResBlockModulateFinal(channel_out, channel_out)
        self.conv22 = ResBlockModulateFinal(channel_out, channel_out)
        self.final = nn.Conv3d(in_channels=channel_out,
                               out_channels=1, kernel_size=1, stride=1, padding=0)
        self.act = Swish()
