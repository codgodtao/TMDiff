...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        if self.opt['phase'] == 'train':  # 如果有预训练模型，之前初始化的参数，这里初始化的scheduler和optG会被覆盖掉
            ...
            self.scheduler = lr_scheduler.StepLR(self.optG, step_size=100000000)
    ...
    def optimize_parameters(self, prompt="QB"):
        self.optG.zero_grad()
        l_pix = self.netG(self.data, prompt).sum()  # multi-gpu
        l_pix.backward()
        self.optG.step()
        self.log_dict['l_pix'] = l_pix.item()

    def test(self, continous=False, prompt="QB"):  # 传入的continous为True
        self.netG.eval()
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                self.SR = self.netG.module.super_resolution(
                    self.data, continous, prompt)
            else:
