...
class ControlNet(nn.Module):
    def __init__(
        ...
        self.tasks_to_id = {"control_hed": 0, "control_canny": 1, "control_seg": 2, "control_depth": 3,
                            "control_normal": 4, "control_openpose": 5, "control_img": 6, "control_hedsketch": 7,
                            "control_bbox": 8, "control_outpainting": 9, "control_grayscale": 10, "control_blur": 11,
                            "control_inpainting": 12}
        ...
        self.input_hint_block_zeroconv_0 = nn.ModuleList(
            [zero_module(conv_nd(dims, 32, 32, 3, padding=1)), zero_module(conv_nd(dims, 32, 32, 3, padding=1))])
        ...
        self.input_hint_block_zeroconv_1 = nn.ModuleList([zero_module(conv_nd(dims, 256, model_channels, 3, padding=1)),
                                                          zero_module(
    ...
    def forward(self, x, hint, timesteps, context, **kwargs):
        ...
        guided_hint = modulated_conv2d(guided_hint, self.input_hint_block_zeroconv_0[0].weight,
                                       self.task_id_layernet_zeroconv_0(task_id_emb).repeat(BS_Real, 1).detach(),
                                       padding=1)
        ...
        guided_hint = modulated_conv2d(guided_hint, self.input_hint_block_zeroconv_1[0].weight,
                                       self.task_id_layernet_zeroconv_1(task_id_emb).repeat(BS_Real, 1).detach(),
                                       padding=1)
        ...
        for module, zero_conv, task_hyperlayer in zip(self.input_blocks, self.zero_convs, self.task_id_layernet):
            ...
            outs.append(
                modulated_conv2d(h, zero_conv[0].weight, task_hyperlayer(task_id_emb).repeat(BS_Real, 1).detach()) +
                zero_conv[0].bias.unsqueeze(0).unsqueeze(2).unsqueeze(3))
...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(self, control_stage_config, control_key, only_mid_control, *args, **kwargs):
        ...
        self.mapping_task = {"control_hed": "hed edge to image", "control_canny": "canny edge to image",
                             ...
                             "control_blur": "deblur image to clean image", "control_inpainting": "image inpainting"}
...
"""
使用了CLIP embeding的last layer batch 768的一个tensor
(task_id_hypernet): Sequential(
      (0): Linear(in_features=768, out_features=1280, bias=True)
      (1): SiLU()
      (2): Linear(in_features=1280, out_features=1280, bias=True)
      (3): SiLU()
    )
(task_id_layernet_zeroconv_0): Linear(in_features=1280, out_features=32, bias=True)
(task_id_layernet_zeroconv_1): Linear(in_features=1280, out_features=256, bias=True)
(task_id_layernet): ModuleList(
      ...
      (7): Linear(in_features=1280, out_features=1280, bias=True)
      (8): Linear(in_features=1280, out_features=1280, bias=True)
      (9): Linear(in_features=1280, out_features=1280, bias=True)
      (10): Linear(in_features=1280, out_features=1280, bias=True)
      (11): Linear(in_features=1280, out_features=1280, bias=True)
    )
    先定义卷积层  self.input_hint_block_zeroconv_1  随后通过函数输出weighted result+bias,修改forward函数就行
    guided_hint = modulated_conv2d(guided_hint, self.input_hint_block_zeroconv_1[0].weight,
                                       self.task_id_layernet_zeroconv_1(task_id_emb).repeat(BS_Real, 1).detach(),
                                       padding=1)
    guided_hint += self.input_hint_block_zeroconv_1[0].bias.unsqueeze(0).unsqueeze(2).unsqueeze(3)
