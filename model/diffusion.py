...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised=True, x_in=None, w=3.0):
        ...
        if x_in is not None:
            x_recon = self.predict_start_from_noise(  # 根据噪声得到x0
                x, t=t, noise=self.denoise_fn(x_t=x, t_input=time_in, MS=x_in['MS'], PAN=x_in['PAN']))
    ...
    #     device = self.betas.device
    #     x_T = torch.randn(x_in['HR'].shape, device=device)
    #     condition = x_in['MS']
    ...
    def p_losses_dynamic(self, x_in, noise=None):
        # 训练过程使用的L1损失函数
        x_start = x_in['HR']
        [b, c, h, w] = x_start.shape
        time_in = torch.from_numpy(np.random.randint(1, self.num_timesteps + 1, size=b))  # 随机选取一个timestamp索引1-T进行加噪
        ...
        x_recon = self.denoise_fn(x_noisy, time_in, x_in['MS'], x_in['PAN'])
        loss = self.epsilon_loss(noise, x_recon)
        return loss

    def p_losses_dynamic_origin(self, x_in, noise=None):
        # 训练过程使用的L1损失函数
        x_start = x_in['HR']
        [b, c, h, w] = x_start.shape
        time_in = torch.from_numpy(np.random.randint(1, self.num_timesteps + 1, size=b))  # 随机选取一个timestamp索引1-T进行加噪
